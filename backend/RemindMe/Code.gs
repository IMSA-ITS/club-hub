...
//reminderSSKey: key for spreadsheet with reminders
//folderId: key for folder to store posters.
var reminderSSKey = "<<ENTER YOUR SPREADSHEET KEY HERE>>";
var submissionSSKey = "<<ENTER YOUR SPREADSHEET KEY HERE>>";
...
//Send out a email digest to everyone with a reminder. Opt-in.
function dispatchEmails() {
        ...
        for (i = 0; i < values.length; i++) {
                ...
                for (a = 0; a < maillist.length; a++) {
                        ...
                        posterbody += "<b><u>" + String(postersheet.getRange(maillist[a] + 1, 3).getValue()) + " (by " + String(postersheet.getRange(maillist[a] + 1, 2).getValue()) + ")</b></u><br />";
                ...
                if (posterbody != "") {
                        ...
                        if (maillist.length > 2) {
                                var subject = "[" + datetoday + "] Club Hub Today: " + String(postersheet.getRange(maillist[0] + 1, 3).getValue()) + ", " + String(postersheet.getRange(maillist[1] + 1, 3).getValue()) + ", and more!";
                        } else if (maillist.length == 2) {
                                var subject = "[" + datetoday + "] Club Hub Today: " + String(postersheet.getRange(maillist[0] + 1, 3).getValue()) + " and " + String(postersheet.getRange(maillist[1] + 1, 3).getValue());
                        } else if (maillist.length == 1) {
                                var subject = "[" + datetoday + "] Club Hub Today: " + String(postersheet.getRange(maillist[0] + 1, 3).getValue());
                        }


                        var header = "<p>Here's what's happening at Club Hub Today! For more details and events, see it all at <a href=\"https://studco.org/clubhub\">studco.org/clubhub</a>.<br /><br />";
                        var footer = "<p>...and those are your events for today! Thank you for using the Club Hub RemindMe! Service.</p><i><font color=\"gray\">This service is in beta. Please report bugs to your Club Hub administrator.</i>";
